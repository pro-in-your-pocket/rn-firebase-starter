...
const DEFAULT_COMPANY_NAME = 'proinyourpocket';
const DEFAULT_PACKAGE_NAME = 'com.proinyourpocket.fbstarter';
const DEFAULT_PROJECT_NAME = 'RNFirebaseStarter';
const VALID_CHARACTERS = /^[a-zA-Z\s]+$/;
const VALID_PACKAGE_CHARACTERS = /^[a-zA-Z\s.]+$/;
...
const run = async () => {
    ...
    let packageName = await readInput('Package name, e.g. com.mycompany.product');
    console.log('---------------------------------------------------------');
    if (!companyName || companyName === '' || companyName.trim() === '') {
        packageName = `com.${companyName}.${projectName.toLowerCase()}`;
        console.log('Using ', packageName)
    }
    if (!packageName.match(VALID_PACKAGE_CHARACTERS)) {
        throw new Error('ERROR: The package name must only contain letters or spaces and periods (.)');
    }

    projectName = projectName.replace(/ /g, '');
    companyName = companyName.replace(/ /g, '').toLowerCase();
    packageName = packageName.replace(/ /g, '').toLowerCase();
